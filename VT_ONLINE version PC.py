...
def Database():
    ...
    cursor.executescript('''
        CREATE TABLE IF NOT EXISTS user (users_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, login TEXT, password TEXT, prenom TEXT, nom TEXT, adresse TEXT, code TEXT, ville TEXT, tel TEXT, email TEXT, type TEXT);
        CREATE TABLE IF NOT EXISTS horaire (hor_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, users_id INTEGER, jour TEXT, debut_AM TEXT, fin_AM TEXT, debut_PM TEXT, fin_PM TEXT);
        CREATE TABLE IF NOT EXISTS rdvs (rdvs_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_vt INTEGER, login_client TEXT, jour_rdvs TEXT, heure TEXT, animal TEXT, type_consult TEXT);
    ''')
...
def utilisateur(login):
    ...
    # RECUPERATION DES CODES POSTAUX DANS TABLE CP
    ...
    frame5.grid(row=12, columnspan=4, pady=25, padx=15)
         ...
         ############################################################
         #  FRAME 4 : AFFICHE PLUSIEURS VETE AVEC LE MEME CODE POSTAL
         ############################################################
    ...
    def RESULTS():
        ...
        ############################################################
        # FRAME 5 : AFFICHE LE VETERINAIRE CHOISI PAR L'UTILISATEUR
        ############################################################
        ...
        def AFFICHE():
            ...
            # == == == == == NOM DU VETERINAIRE selectionné == == == == ==
            lbl1 = Label(frame5, text="Vous avez sélectionné", fg="blue", font="Verdana 18")
            lbl1.grid(row=1, column=1, columnspan=3, pady=0, padx=0)
            lbl2 = Label(frame5, text=f"{Nom}", fg="grey", font="Verdana 14")
            lbl2.grid(row=2, column=2, pady=0, padx=0)
            lbl3 = Label(frame5, text=f"{rslt[0]} à  {rslt[1]} {rslt[2]}", fg="grey", font="Verdana 14")
            lbl3.grid(row=3, column=2, pady=0, padx=10)
            lbl4 = Label(frame5, text=("Pour votre " + a + " pour " + b), fg="black", font="Verdana 14")
            lbl4.grid(row=4, column=2, pady=0, padx=10)


            # == == == =PHOTO DU VETERINAIRE == == == == ==
            ...
            ###########################################################################################################
            #                             OUVERTURE DU PLANING DU VETERINAIRE CHOISI
            ###########################################################################################################
            def REQUETE(user_id):
                vt = Toplevel()
                ...
                lbl1 = Label(vt, text="Voici les horaires de consultation de : " + Nom, fg="black", bg="orange",font="Verdana 22")
                lbl1.place(x=50, y=100)

                frame6 = Frame(vt)
                frame6.place(x=50, y=680)
                frame6['bg'] = "orange"
                ...
                #########################################################################
                # AFFICHAGE DU GRID AVEC LES HEURES DE CONSULTATIONS DU VT SELECTIONNE
                #########################################################################
                ...
                ###########################################################################
                # FINALISATION DU RENDEZ-VOUS BOUTON : CHOIX DU JOUR ET DE L'HEURE
                ##########################################################################
                lblR = Label(vt, text="Pour prendre rendez-vous : ", fg="black", bg="orange", font="Verdana 22")
                lblR.place(x=100, y=600)

                vlist5 = ["lundi", "mardi", "mercredi", "jeudi", "vendredi"]
                Combo5 = ttk.Combobox(vt, values=vlist5, font="Verdana 16", width=18)
                Combo5.set("Choisissez votre JOUR")
                Combo5.place(x=520, y=605)
                Combo5.bind('<<ComboboxSelected>>', lambda e: print(Combo5.get()))

                lbl4 = Label(vt, text="Votre heure : ", fg="black", bg="orange", font="Verdana 22")
                lbl4.place(x=800, y=600)
                entryheure = Entry(vt, font=('arial', 20), width=5)
                entryheure.place(x=1000, y=600)

                ##############################CONFIRMATION DU RDVS (BUTTON : prendre rendez-vous ) ##########################
                def CONFIRMATION():
                    Jour = Combo5.get()
                    Heure = entryheure.get()
                    animal=Combo1.get()
                    type_consult = Combo2.get()
                    lbl8 = Label(frame6, text=f"Vous avez rendez-vous avec :  {Nom} à {rslt[0]} - {rslt[1]} {rslt[2]} ",
                                 fg="black", bg="orange", font="Verdana 18")
                    lbl8.pack(pady=15)
                    lbl9 = Label(frame6, text=f"Le {Jour} à {Heure} heures", fg="black", bg="orange", font="Verdana 20")
                    lbl9.pack(pady=0)
                    cursor.execute(
                        "INSERT INTO rdvs (user_vt, login_client, jour_rdvs, heure, animal, type_consult) VALUES(?, ?, ?, ?, ?, ?)",
                        (rslt[3], login, Jour, Heure, animal, type_consult))
                    conn.commit()



                button = Button(vt, text="Prendre rendez-vous", command=CONFIRMATION, fg="blue", font="Verdana 14",
                                bd=2, bg="orange", relief="groove")
                button.place(x=840, y=650)
                vt.mainloop()
...
def staff(login):
    vt = Toplevel()
    vt.title("Planing Consultation")
    vt.geometry("1400x960+10+20")
    vt.minsize(480, 360)
    vt['bg'] = "orange"

    lbl_titre = Label(vt, text="Bienvenue dans l'application VT_ONLINE", fg="white", bg="grey", font="Verdana 34")
    lbl_titre.pack(pady=10)
    lbl2 = Label(vt, text= "" , fg="black", bg="orange", font="Verdana 20")
    lbl2.pack(pady=10)
    lbl3 = Label(vt, text="" , fg="black", bg="orange", font="Verdana 20")
    lbl3.pack(pady=10)
    lbl4 = Label(vt, text="" , fg="black", bg="orange", font="Verdana 20")
    lbl4.pack(pady=10)
    ...
    lbl3.config(text=f"Confirmez votre identité : vous êtes bien {vet[1]} {vet[2]} habitant à ")
    lbl4.config(text=f" {vet[3]} à  {vet[4]} {vet[5]} ?")
    ...
    width = 500
    height = 300
    imageVT2 = PhotoImage(file="Image3.png").zoom(9).subsample(10)
    canvas1 = Canvas(frameR, width=width, height=height, bg='orange', bd=0, highlightthickness=0)
    canvas1.create_image(width / 2, height / 2, image=imageVT2)
    ...
    lbl_sstitre = Label(vt, text="Choisissez vos jours de consultations et vos horaires", fg="black", bg="orange",
                        font="Verdana 22")
    lbl_sstitre.place(x=40, y=300)

    label = Label(FrameL, text="                 AM                      PM                 ", font=("Arial", 20),
                  bg="grey", fg="white")
    label.grid(row=0, column=2, columnspan=4, pady=0, padx=18)
    ...
    def VALIDATE():
        ...
        lblVal = Label(vt, text="Vos jours de consultation ont bien été enregistrés !", fg="red", bg="orange", font="Verdana 20")
        lblVal.place(x=520, y=850)


    lbl = Label(vt, text="Voulez-vous enregistrer vos modifications ?", fg="black", bg="orange", font="Verdana 20")
    lbl.place(x=60, y=800)
    button_valider = Button(vt, text="Valider", command=VALIDATE, fg="black", font="Verdana 20", bd=2, bg="light blue",
                            relief="groove")
    button_valider.place(x=700, y=800)
    ...
    def Planing():
        ...
        print(users_id)
        print(login)
        cursor.execute(f"SELECT jour_rdvs, heure, animal, type_consult, login_client FROM rdvs WHERE user_vt = '{users_id}'")
        rdvs = cursor.fetchone()

        client = cursor.execute(f"SELECT nom, prenom, tel, email FROM user WHERE login = '{rdvs[4]}'")
        nom_client = client.fetchone()
        ...
        lbl2.pack(pady=10)
        lbl3 = Label(fen, text=f"Vous avez rendez-vous le {rdvs[0]} à {rdvs[1]} h, pour {rdvs[3]} ", fg="black", bg="orange", font="Verdana 20")
        lbl3.pack(pady=10)
        lbl4 = Label(fen, text=f"Avec animal : {rdvs[2]}  de {nom_client[0]} {nom_client[1]}", fg="black", bg="orange", font="Verdana 20")
        lbl4.pack(pady=10)
        lbl5 = Label(fen, text=f"Tél : {nom_client[2]}. Email : {nom_client[3]}", fg="black", bg="orange", font="Verdana 20")
        lbl5.pack(pady=10)
        conn.commit()


    lbl_check = Button(vt, text="Voir vos rendez-vous", command=Planing, fg="Blue", font=('arial', 22))
    lbl_check.place(x=850, y=800)
