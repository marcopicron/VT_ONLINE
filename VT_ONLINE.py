    ...
    def RESULTS():
        ...
        def AFFICHE():
            ...
            def REQUETE(user_id):
                ...
                lbl2 = Label(vt, text=f"{rslt[0]} à {rslt[1]} {rslt[2]} ", fg="grey", font="Verdana 18")
...
def staff():
    ...
    lbl2 = Label(vt, text="Vous êtes : ", fg="grey", font="Verdana 18")
    lbl2.pack(pady=10)
    ...
    ######################################
    FrameL = Frame(vt)
    FrameL.pack(side=LEFT, pady=0, padx=300)
    ...
    def VALIDATE():
        l = cursor.execute(f"SELECT users_id FROM user WHERE login = {login}")
        users_id = l.fetchone()
        print (users_id)
        ...
        ######################################
        # cursor.execute("SELECT users_id, nom, prenom FROM user WHERE login = ?", LOGIN.get())
        # rslt = cursor.fetchone()
        # print(rslt)
        # lbl2.config(text=f"Vous êtes {rslt[1]} {rslt[2]}")
        # lbl3.config(text=f"Votre id est le : {rslt[0]} ")
